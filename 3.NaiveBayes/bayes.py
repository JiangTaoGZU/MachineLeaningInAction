...
def testing_naive_bayes():
    ...
    print("测试数据为:",test_one)
    print('分类结果为 {}'.format(classify_naive_bayes(test_one_doc, p0v, p1v, p_abusive)))
    test_two = ['stupid' , 'stupid' , 'stupid' , 'garbage']
    test_two_doc = np.array(bag_words2vec(vocab_list, test_two))
    print("测试数据为:", test_two)
    print('分类结果为: {}'.format(classify_naive_bayes(test_two_doc, p0v, p1v, p_abusive)))

def bag_words2vec(vocab_list, input_set):
    # 注意和原来的set_of_words2vec做对比
    result = [0] * len(vocab_list)
    for word in input_set:
        if word in vocab_list:
            result[vocab_list.index(word)] += 1
...
def spam_test():
    ...
    for i in range(1, 26):
        ...
        try:
            ...
            words = text_parse(open('data/email/spam/{}.txt'.format(i), encoding='Windows 1252').read())
        ...
        try:
            ...
            words = text_parse(open('data/email/ham/{}.txt'.format(i), encoding='Windows 1252').read())
        doc_list.append(words)
        full_text.extend(words)
        class_list.append(0)
    # 创建词汇表
