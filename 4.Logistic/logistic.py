...
def grad_ascent(data_arr, class_labels):
    ...
    # weights 代表回归系数， 此处的 ones((n,1)) 创建一个行数和特征数相同的矩阵，其中的数全部都是 1
    weights = np.ones((n, 1))

    for k in range(max_cycles):
        #
        #
        # *3 的矩阵 * 3*1 的单位矩阵 ＝ m*1的矩阵
...
def stoc_grad_ascent1(data_mat, class_labels, num_iter=150):
    """
    改进版的随机梯度上升，每次迭代使用随机的一个样本来更新回归系数
    :param data_mat: 输入数据的数据特征（除去最后一列）,ndarray
    :param class_labels: 输入数据的类别标签（最后一列数据
    :param num_iter: 迭代次数
    :return: 得到的最佳回归系数
...
def multi_test():
    ...
    print('在{} 次迭代后的平均错误率是 {}'.format(num_tests, error_sum / num_tests))
