...
def stageWise(xArr, yArr, eps=0.01, numIt=100):
    ...
    for i in range(numIt):
        print("第%d次迭代的系数结果是:%s"%(i+1,ws.T))
        lowestError = inf
        for j in range(n):
            for sign in [-1, 1]:
                wsTest = ws.copy()
                #eps表示每次迭代需要调整的步长,sign取-1或1,即是在分别计算增加或者减少该特征对误差的影响
                wsTest[j] += eps * sign  
                yTest = xMat * wsTest
...
def scrapePage(retX, retY, inFile, yr, numPce, origPrc): 
    ...
    while(len(currentRow)!=0):
        ...
        if len(soldUnicde)==0:
            ...
            # 去掉不完整的套装价格(这里用了启发式的方法,就是用经验设置规则的意思,认为价格比原价低一半以上的就是不完整的)
...
def crossValidation(xArr,yArr,numVal=10):
    ...
    # 计算误差估计值的均值    # 可以在 30 个不同的 lambda 下调用 ridgeRegres() 函数。
    ...
    # 输出构建的模型
    print ("用岭回归模型预测出的最好的一组回归系数是:\n",unReg)
    print ("常数项是: ",-1*sum(multiply(meanX,unReg)) + mean(yMat))
...
def regression2():
    xArr, yArr = loadDataSet("data/8.Regression/data.txt")
    yHat = lwlrTest(xArr, xArr, yArr, 0.003)   #调整k值观察不同画图结果
    xMat = mat(xArr)
...
def abaloneTest():
    ...
    print("标准线性回归的错误值是:", rssError(abY[100:199], standyHat.T.A))
...
def regression4():
    ...
    ws=stageWise(xArr, yArr, 0.005, 200) #5000次以后与最小二乘法结果类似
    #把计算结果与最小二乘法进行比较
    xMat = mat(xArr)
    yMat = mat(yArr).T
    xMat = regularize(xMat)
    ...
    print("标准线性回归得到的回归系数矩阵是:",weights.T)
    #绘制鲍鱼数据集上执行逐步线性回归法与迭代次数的关系图
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.plot(ws)
    plt.show()
...
def regression5():
    lgX = []
    lgY = []
    #对六种乐高套装进行价格等信息的爬取,并把结果存入lgX与lgY
