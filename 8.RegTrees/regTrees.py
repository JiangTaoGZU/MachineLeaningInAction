...
# 计算总方差=方差*样本数
def regErr(dataSet):
    # shape(dataSet)[0] 表示行数,var函数求解方差
...
def chooseBestSplit(dataSet, leafType=regLeaf, errType=regErr, ops=(1, 4)):
    ...
    for featIndex in range(n-1): # 对于每个特征
        ...
        for splitVal in set(dataSet[:, featIndex].T.tolist()[0]):
            ...
            # 如果二元切分，算出来的误差如果更小，那么就记录切分点，并记录最小误差
            if newS < bestS:
                bestIndex = featIndex
                bestValue = splitVal
                bestS = newS
    # 如果下降值小于tolS,则停止划分,返回叶节点
    if (S - bestS) < tolS:
        return None, leafType(dataSet)
...
def createTree(dataSet, leafType=regLeaf, errType=regErr, ops=(1, 4)):
    """createTree(获取回归树)
        Description：递归函数：如果构建的是回归树，该模型是一个常数，如果是模型树，其模型是一个线性方程。
    Args:
        dataSet      加载的原始数据集
        leafType     建立叶子点的函数
        errType      误差计算函数
...
# 此函数自顶向下遍历整棵树，直到命中叶节点为止，一旦到达叶节点，它就会在输入数据上S
...
#----------------模型树-----------------
    ...
    print("data4的数据得到的模型树是:\n",myTree)

#----------------回归树 VS 模型树 VS 线性回归自行车速度智商案例-----------------   .
    print("="*80)
    ...
    print("回归树R2:", corrcoef(yHat1, testMat[:, 1],rowvar=0)[0, 1])

    # 模型树
    myTree2 = createTree(trainMat, modelLeaf, modelErr, ops=(1, 20))
    yHat2 = createForeCast(myTree2, testMat[:, 0], modelTreeEval)
    print("模型树是:\n",myTree2)
    print("模型树R2:", corrcoef(yHat2, testMat[:, 1],rowvar=0)[0, 1])
    ...
    print("线性回归R2:", corrcoef(yHat3, testMat[:, 1],rowvar=0)[0, 1])
